/**
 * Development content script with Hot Module Replacement (HMR) support for CSS changes.
...
// Development-specific UI setup with HMR support
...
if (!styleElement) {
  console.warn(
    'Chrome Markdownify Dev: Style element with attribute data-vite-dev-id not found.'
  );
}
...
try {
  ...
  console.warn(
    'Chrome Markdownify Dev: Could not create shadow root UI, continuing without UI',
    error
  );
}
...
chrome.runtime.onMessage.addListener(
  (
    ...
    if (debugMode) {
      console.log(
        'Chrome Markdownify Content Script (Dev): Received message:',
        request
      );
    }
    ...
    try {
      ...
      console.error(
        'Chrome Markdownify Content Script (Dev): Error handling message:',
        error
      );
      sendResponse({
        success: false,
...
async function handleFullPageRequest(
  ...
  try {
    ...
    if (debugMode) {
      console.log(
        'Chrome Markdownify Content Script (Dev): Full page converted successfully',
        {
    ...
    console.error(
      'Chrome Markdownify Content Script (Dev): Error converting full page:',
      error
    );
    sendResponse({
      success: false,
...
async function handleSelectionRequest(
  ...
  try {
    ...
    if (debugMode) {
      console.log(
        'Chrome Markdownify Content Script (Dev): Selection converted successfully',
        {
    ...
    console.error(
      'Chrome Markdownify Content Script (Dev): Error converting selection:',
      error
    );
    sendResponse({
      success: false,
...
function handleCheckSelectionRequest(
  ...
  try {
    ...
    if (debugMode) {
      console.log('Chrome Markdownify Content Script (Dev): Selection check:', {
        hasSelection,
      });
    }
  } catch (error) {
    console.error(
      'Chrome Markdownify Content Script (Dev): Error checking selection:',
      error
    );
    sendResponse({
      success: false,
...
function checkAndUpdateSelection(): void {
  ...
  if (hasSelection !== lastSelectionState) {
    ...
    if (debugMode) {
      console.log(
        'Chrome Markdownify Content Script (Dev): Selection state changed:',
        hasSelection
      );
    }
  }
...
/**
 * Initialize content script with development features
 */
function initContentScript(): void {
  // Check if we're in a valid context
  if (typeof window === 'undefined' || !document) {
    console.warn(
      'Chrome Markdownify Content Script (Dev): Invalid context, skipping initialization'
    );
    return;
  }
  ...
  if (
    ...
    if (debugMode) {
      console.log(
        'Chrome Markdownify Content Script (Dev): Skipping restricted page'
      );
    }
    return;
  }
  ...
  if (debugMode) {
    console.log(
      'Chrome Markdownify Content Script (Dev): Initialized successfully',
      {
...
// Development-specific HMR handling
if (import.meta.hot) {
  import.meta.hot.accept('./Content', () => {
    console.log('Chrome Markdownify (Dev): Content component hot-reloaded');
    // Re-render with new Content component
    if (root) {
      root.render(<Content />);
    }
  });

  import.meta.hot.accept(['@utils/dom-extractor', '@utils/converter'], () => {
    console.log('Chrome Markdownify (Dev): Utilities hot-reloaded');
  });
}
